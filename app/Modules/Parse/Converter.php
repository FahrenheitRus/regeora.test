...
use App\Modules\Parse\Exceptions\ParseValidationException;
...
class Converter
{
     ...
     * @return array
     * @throws ParseLogicException
     * @throws ParseValidationException
     */
    public function getConvertedProp(array $prop, array $rules)
    {
        foreach ($rules as $import_prop => $object_prop) {
            ...
            if (!isset($prop[$import_prop])) {
                if (isset($object_prop['default'])) {
                    ...
                    throw new ParseValidationException('prop is required');
                }
            } else {
    ...
    public function getStripedProp(array $prop, array $rules)
    {
    ...
    public function getPreparedRules(array $rules)
    {
        $rules_new = [];
        foreach ($rules as $import_prop => $object_prop) {
            ...
            if (empty($object_prop['prop'])) {
                ...
                if (is_numeric($import_prop)) {
                    throw new ParseLogicException('getPreparedRules: !prop and !key');
                }
     ...
     * @throws ParseValidationException
     */
    public function convertTimeToMinute(string $time)
    {
        if (!preg_match('~^0?(\d+):0?(\d+)$~', $time, $m)) {
            throw new ParseValidationException("!time: $time");
        }
     ...
     * @throws ParseValidationException
     */
    public function convertDistanceKmToM($distance)
    {
        if (!is_numeric($distance)) {
            throw new ParseValidationException("!distance: $distance");
        }
